...
function buyReward(taskid, direction, next){
    if(data.tasks[taskid].type != "reward"){
        fnLog("attempted to buy a reward that wasn't a reward");
        return;
    }
    ...
    request.post(apiUrl + "/user/tasks/" + taskid + "/" + direction)
        ...
        .set('X-API-Key', apiToken).end(function(error,res){
            if(error){
                fnLog("Buy reward: " + error, 1);
                setConnected(false);
            } else {
                fnLog("Successfully bought reward");
                setConnected(true);
                setUnsaved(false);
            }
            if(typeof next === "function"){
